...
- **No periodic worker yet.** `send_reminders` and `finalize_bookings` are management commands; they need to be hooked up to cron (or Django-Q2 in Phase 2) before they actually run on a schedule in production.
- **Email verification stays "optional" in MVP.** Per allauth config in [config/settings.py](config/settings.py); flip to `"mandatory"` before public launch to filter spam signups.
- **Allauth templates unstyled.** Login / signup / password-reset render Allauth's default markup inside our chrome — functional but visually rough. Override `templates/account/{login,signup,password_reset}.html` for polish.
- **No rate limiting** on signup / login / booking-create. Add `django-ratelimit` before public launch.
- **No WhiteNoise** for static file serving. Static files only work in `DEBUG=True`; production deploy needs WhiteNoise (or platform CDN) configured.
